#!/usr/bin/env python3
"""Build the five SAT v1.3.0 manuscript figures with Python/matplotlib."""
...
def figure1(out: Path) -> tuple[dict[str, str], list[dict[str, object]]]:
    ...
    ax2.text(0.07, 1.02, "Two geometric endpoints bound recoverable support", fontsize=8.5, fontweight="bold")
    box(ax2, 0.02, 0.44, 0.27, 0.30, LIGHT_RED, "One-to-one", "one QA record")
    box(ax2, 0.36, 0.44, 0.27, 0.30, LIGHT_TEAL, "Set coverage", "all memberships")
    box(ax2, 0.70, 0.44, 0.27, 0.30, LIGHT_GREY, "Human recovery", "not measured")
    arrow(ax2, (0.30, 0.59), (0.35, 0.59), TEAL)
    arrow(ax2, (0.64, 0.59), (0.69, 0.59), MUTED, dashed=True)
    ax2.text(0.155, 0.18, "lower endpoint", ha="center", fontsize=5.8, color=RED, fontweight="bold")
    ax2.text(0.495, 0.18, "geometric upper endpoint", ha="center", fontsize=5.8, color=TEAL, fontweight="bold")
    ax2.text(0.835, 0.18, "plant counts needed", ha="center", fontsize=5.8, color=MUTED, fontweight="bold")
    ...
    rows += [
        {"panel": "b", "endpoint": "one_to_one", "interpretation": "lower_endpoint_one_region_one_record"},
        {"panel": "b", "endpoint": "set_coverage", "interpretation": "geometric_upper_endpoint"},
    ]
...
def main() -> None:
    ...
    v13 = repo / "reproducibility" / "v1_3_0"
