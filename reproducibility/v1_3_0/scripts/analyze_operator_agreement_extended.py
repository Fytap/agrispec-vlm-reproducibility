...
def main() -> None:
    ...
    intersection = human_mask[("consensus_yes", "mask_positive")]
    human_positive = sum(human_mask[("consensus_yes", mask)] for mask in ("mask_positive", "mask_negative"))
    mask_positive = sum(human_mask[(human, "mask_positive")] for human in ("consensus_yes", "consensus_no", "disputed_or_uncertain"))
    union = human_positive + mask_positive - intersection
    human_mask_metrics = [{
        "intersection": intersection,
        "human_consensus_positive": human_positive,
        "mask_positive": mask_positive,
        "mask_precision_against_human_consensus": intersection / mask_positive,
        "mask_recall_against_human_consensus": intersection / human_positive,
        "f1": 2 * intersection / (mask_positive + human_positive),
        "jaccard": intersection / union,
        "unresolved_human_items": sum(human_mask[("disputed_or_uncertain", mask)] for mask in ("mask_positive", "mask_negative")),
    }]
