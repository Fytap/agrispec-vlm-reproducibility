#!/usr/bin/env python3
"""Build revision-7 SAT figures from official-spatial experiment TSV files."""
...
plt.rcParams.update({"font.size": 7, "axes.linewidth": 0.7, "axes.spines.top": False, "axes.spines.right": False, "legend.frameon": False, "xtick.major.width": 0.7, "ytick.major.width": 0.7})
...
def workflow_figure(output: Path) -> tuple[dict[str, str], list[dict[str, object]]]:
    ...
    boxes = [
        ...
        (0.78, 0.69, 0.19, 0.18, "Fixed-budget review queue\nK = 1, 5, 10, 20, 50", "#E4CCD8"),
        (0.05, 0.25, 0.22, 0.20, "Official train (104 tiles)\nselect T070/A032; fit rankers\n300-label simulations", "#DCEBFA"),
        (0.38, 0.25, 0.22, 0.20, "Official validation (26)\nselect C or best epoch\nno final reporting choice", "#E5E5EF"),
        (0.71, 0.25, 0.24, 0.20, "Official test (26)\nscore once after selection\n1,712 raster weed instances", "#F5E1E1"),
    ]
    for x, y, width, height, text, color in boxes:
        ...
        ax.text(x + width / 2, y + height / 2, text, ha="center", va="center", fontsize=7.1)
...
def proposal_figure(root: Path, output: Path) -> tuple[dict[str, str], dict[str, pd.DataFrame]]:
    ...
    fig, axes = plt.subplots(1, 3, figsize=(7.2, 2.55), gridspec_kw={"width_ratios": [1.0, 1.45, 1.0]})
    ...
    for i, value in enumerate(b.candidates_per_image_mean): axes[0].text(i, 0.62, f"{value:.1f} proposals/image", ha="center", fontsize=6, color="#555555")
