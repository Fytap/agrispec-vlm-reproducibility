#!/usr/bin/env python3
"""Rebuild SAT Figures 1--5 with collision-safe final-size typography.

The script preserves every plotted value and source-data table from the registered
Revision-8/10 analyses.  It changes only information architecture, line wrapping,
legend placement, and export QA.
"""
...
def queue_figure(root: Path, output: Path) -> tuple[dict[str, str], dict[str, pd.DataFrame], dict[str, object]]:
    ...
    for model, label, color in models:
        ...
        line = axes[0, 0].plot(current.k, current.all_candidate_queue_precision, marker="o", ms=3.5, lw=1.5, color=color, label=label)[0]
        axes[0, 1].plot(current.k, current.queued_role_qualified_weed_instance_recall, marker="o", ms=3.5, lw=1.5, color=color)
    ...
    for ax, ylabel, label in ((axes[0, 0], "All-candidate queue precision", "a"), (axes[0, 1], "Role-qualified weed recall", "b")):
        ...
        panel(ax, label)
    fig.legend(legend_handles, [label for _, label, _ in models], loc="upper center", bbox_to_anchor=(0.5, 0.995), ncol=5, fontsize=6.4, handlelength=1.7)
    ...
    for i, value in enumerate(values):
        axes[1, 0].text(i, value + 0.009, f"{value:.3f}", ha="center", fontsize=6.3)
...
def robustness_figure(root: Path, output: Path) -> tuple[dict[str, str], dict[str, pd.DataFrame], dict[str, object]]:
    ...
    for (row, column), value in np.ndenumerate(matrix.to_numpy()):
        axes[1, 0].text(column, row, f"{value:.3f}", ha="center", va="center", fontsize=6.3, color="white" if value > 0.16 else "black")
    ...
    panel(axes[1, 1], "d")
    axes[1, 1].text(0, np.quantile(random_k20, 0.975) + 0.008, f"95% random\n{np.quantile(random_k20, 0.025):.3f}--{np.quantile(random_k20, 0.975):.3f}", ha="center", va="bottom", fontsize=6.0, bbox={"facecolor": "white", "edgecolor": "none", "pad": 0.4})
...
def allocation_figure(audit: Path, output: Path) -> tuple[dict[str, str], dict[str, pd.DataFrame], dict[str, object]]:
    ...
    for i, (policy, label, color) in enumerate(zip(POLICIES, POLICY_LABELS, POLICY_COLORS)):
        ...
        for bar, value in zip(bars, values):
            axa.text(bar.get_x() + bar.get_width() / 2, value + 0.013, f"{value:.2f}", ha="center", va="bottom", fontsize=5.7, rotation=90)
    axa.set_xticks(x, metric_labels)
    axa.set_ylim(0, 0.90)
    axa.set_ylabel("Proportion")
    axa.legend(ncol=2, loc="lower center", bbox_to_anchor=(0.5, 1.01), fontsize=6.2, handlelength=1.2, labelspacing=0.2, borderaxespad=0.0)
    panel(axa, "a")
    ...
    axc.legend(loc="upper left", fontsize=6.2)
    panel(axc, "c")
    ...
    axd.legend(loc="lower center", bbox_to_anchor=(0.5, 1.01), fontsize=6.2, ncol=1, borderaxespad=0.0, labelspacing=0.2)
    panel(axd, "d")
