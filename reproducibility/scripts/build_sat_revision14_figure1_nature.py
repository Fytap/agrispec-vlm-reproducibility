#!/usr/bin/env python3
"""Build the Figure 1 information architecture for the SAT paper.
...
def draw_panel_a(ax: plt.Axes) -> None:
    ...
    ax.axis("off")
    panel_label(ax, "a")
    ax.text(0.0, 0.985, "Support-aware review-queue evaluation", fontsize=8.5, fontweight="bold", color=PALETTE["ink"], va="top")
    ax.text(0.0, 0.915, "RGB observations move through candidate formation, qualification, ranking, and allocation", fontsize=6.2, color=PALETTE["muted"], va="top")
    rounded_box(ax, 0.565, 0.840, 0.195, 0.075, PALETTE["aqua"], edgecolor="none", linewidth=0, radius=0.025)
    ax.text(0.6625, 0.888, "COMPUTATIONAL QUEUE", fontsize=5.35, fontweight="bold", color=PALETTE["teal"], va="center", ha="center")
    ax.text(0.6625, 0.858, "proposal · rank · allocation", fontsize=5.0, color=PALETTE["muted"], va="center", ha="center")
    rounded_box(ax, 0.785, 0.840, 0.200, 0.075, PALETTE["lilac"], edgecolor="none", linewidth=0, radius=0.025)
    ax.text(0.885, 0.888, "GROUP AUDIT", fontsize=5.35, fontweight="bold", color=PALETTE["violet"], va="center", ha="center")
    ax.text(0.885, 0.858, "independent ratings · consensus", fontsize=5.0, color=PALETTE["muted"], va="center", ha="center")

    # Remove legacy pilot labels before drawing the group-audit header.
    for artist in list(ax.texts):
        if artist.get_text() == "GROUP AUDIT" or artist.get_text().startswith("independent ratings"):
            artist.remove()
    rounded_box(ax, 0.785, 0.840, 0.200, 0.075, PALETTE["lilac"], edgecolor="none", linewidth=0, radius=0.025)
    ax.text(0.885, 0.888, "GROUP AUDIT", fontsize=5.35, fontweight="bold", color=PALETTE["violet"], va="center", ha="center")
    ax.text(0.885, 0.858, "date | tile | patch proxy", fontsize=5.0, color=PALETTE["muted"], va="center", ha="center")

    ax.text(0.0, 0.790, "PROPOSAL-TO-REVIEW PATH", fontsize=6.0, fontweight="bold", color=PALETTE["teal"], va="center")
    ...
    node_specs = [
        ...
        (0.755, "Queue\nallocation", "fixed K / quota grid", PALETTE["rose"]),
    ]
    node_w = 0.155
    node_y = 0.345
    node_h = 0.255
    for x, title, subtitle, color in node_specs:
        ...
        ax.text(x + node_w / 2, node_y + node_h * 0.61, title, fontsize=6.8, fontweight="bold", color=PALETTE["ink"], ha="center", va="center", linespacing=1.08)
        ax.text(x + node_w / 2, node_y + node_h * 0.20, subtitle, fontsize=5.35, color=PALETTE["muted"], ha="center", va="center")
    ...
    ax.text(0.951, 0.315, "Queue audit", fontsize=5.35, color=PALETTE["rose_dark"], ha="center", va="top", fontweight="bold")
    ...
    ax.text(0.0, 0.010, "Test truth enters only after each queue is locked; group summaries never alter model settings.", fontsize=5.7, color=PALETTE["rose_dark"], fontweight="bold", va="bottom")
...
def draw_panel_c(ax: plt.Axes) -> None:
    ...
    ax.text(0.0, 0.015, "Test truth enters after queue formation.", fontsize=5.6, fontweight="bold", color=PALETTE["rose_dark"], va="bottom")
...
def write_source_tables(output_dir: Path) -> dict[str, str]:
    workflow_rows = [
        ...
        ["a", "queue_allocation", "Queue allocation", "fixed K and bounded allocation", "computational queue"],
        ["a", "queue_audit", "Queue audit", "date, tile, and patch-proxy summaries", "offline evaluation"],
        ["a", "target_semantic", "Target-semantic baseline", "alternative component formation", "descriptive comparison"],
    ]
    data_rows = [
