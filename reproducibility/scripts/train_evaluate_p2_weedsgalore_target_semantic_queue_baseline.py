#!/usr/bin/env python3
"""Train and evaluate an equal-supervision target semantic queue baseline.

This reviewer-requested analysis is explicitly post-test diagnostic work.  The
official training split supplies dense-mask supervision.  Epoch and component
settings are selected on official validation by the final K=20 one-to-one
queue metric.  Official-test labels are opened only after that selection and
are never used for threshold, epoch, component, candidate-ID, or queue-policy
selection.
"""
...
class TargetDataset:
    ...
    @staticmethod
    def build(stems: list[str], root: Path, train: bool, seed: int):
        import torch
...
def truth_instances(stem: str, root: Path) -> tuple[np.ndarray, list[TruthInstance]]:
    paths = paths_for_stem(root, stem)
    ...
    for value in np.unique(instance_map):
        ...
        records.append(
            TruthInstance(
                instance_id=f"{stem}:instance:{int(value)}",
...
def component_candidates(
    ...
    semantic, instances = truth_instances(stem, root)
    ...
    for component in range(1, count + 1):
        ...
        for record in instances:
            overlap = int((mask & record.mask).sum())
            if overlap == 0:
                continue
...
def evaluate_probabilities(
    ...
    for stem in sorted(probabilities):
        ...
        _, instances = truth_instances(stem, root)
...
def main() -> None:
    ...
    exists = torch.tensor(int(rank == 0 and output.exists()), device=device)
    ...
    train_dataset = TargetDataset.build(train_stems, dataset_root, train=True, seed=args.seed)
    ...
    model = DDP(build_model().to(device), device_ids=[local_rank], broadcast_buffers=False)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if rank == 0:
            validation_probabilities, validation_seconds, validation_peak = predict(
                model.module, val_stems, dataset_root, device
            )
    ...
    if rank == 0:
        ...
        test_probabilities, test_seconds, test_peak = predict(model.module, test_stems, dataset_root, device)
        ...
        governance = {
            ...
            "reason": "equal-target-supervision semantic segmentation baseline requested after review",
        ...
        result = {
            ...
            "limitations": [
                "This is a post-test diagnostic baseline added after reviewer feedback, not a preregistered confirmatory test.",
                "The official split contains spatial patches from one field and does not support cross-farm inference.",
                "No operator or actuator study was performed.",
            ],
        }
